...
def plot_two_plots(df,column, title1, title2, xlabel1, xlabel2, fontsize, rotation):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20,4))
    
    df_grouped = df.groupby(df['order_purchase_timestamp'].dt.date).mean()
    ax1.plot(df_grouped.index, df_grouped[column])
    ax1.set_title(title1, fontsize=fontsize)
    ax1.set_xlabel(xlabel1, fontsize=fontsize)
    ax1.set_ylabel(column, fontsize=fontsize)
    ...
    ax2.plot(df_grouped.index, df_grouped[column])
    ax2.axhline(y=df_2017[column].mean(), color='red', linestyle='--')
    ax2.set_title(title2, fontsize=fontsize)
    ax2.set_xlabel(xlabel2, fontsize=fontsize)
    ax2.set_ylabel(column, fontsize=fontsize)
